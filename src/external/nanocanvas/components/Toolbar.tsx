...
import { Home, Save, Download, Upload, Type, Square, Circle, Brush, MousePointer, Trash, Hand } from 'lucide-react';
...
const Toolbar: React.FC<ToolbarProps> = ({ activeTool, onSelectTool, onDelete, onDownload, onUploadImage, onSaveProject, onHome, selectedProperties, onUpdateProperty, hasSelection, compact }) => {
  return (
    <div className="absolute left-4 top-4 z-50 flex gap-4 items-start">
      <div className={`flex flex-col bg-[#0B1220]/95 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 w-16`}>
        <div className="px-3 pt-3 pb-2 text-[10px] uppercase tracking-wider text-slate-400">TOOLS</div>
        ...
        <button className={`p-3 ${activeTool === 'move' ? 'bg-indigo-600 text-white' : 'text-slate-300 hover:bg-white/10'}`} onClick={() => onSelectTool('move')} title="Move"><Hand size={20} /></button>
